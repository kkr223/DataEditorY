...
- HMR WebSocket 端口：`43128`

这样可以避免部分 Linux / Arch 环境下使用 `localhost` 与默认低位开发端口时出现的白屏问题。
...
- 开发模式固定使用 `127.0.0.1:43127`，不再使用 `localhost:1420`
- `static/resources/` 下的 Lua 参考资源是可编辑源文件，不应手工同步到 `src/lib/data/lua-intel/`
- Tauri 打包时会另外把它们复制到外置的 `resources/lua-intel/` 目录，便于用户直接修改
- 桌面版运行时会优先读取 `resources/lua-intel/` 下的文件；修改后重新启动应用即可生效
...
其中 `cover.jpg` 作为默认卡图占位图使用，`strings.conf` 用于提供 setcode 名称等基础数据，`resources/lua-intel/` 用于提供 Lua 编辑器可外部维护的函数/常量/snippet 资源，`yugioh-card/` 用于提供制卡器预览和导出所需的静态卡模资源。

说明：

- `base` 变体分发 `strings.conf`、`cover.jpg` 和 `resources/lua-intel/`
...
- 打包后的桌面版会在程序旁的 `resources/lua-intel/` 下额外放出同名文件，方便直接替换和维护
...
- 支持跨数据库复制和粘贴卡片
- 支持数据库级撤回最近操作
- 支持将当前数据库同级的 `.cdb`、`pics/`、`script/` 一键打包为 ZIP
...
点击顶部工具栏中的 `打包 ZIP` 按钮，可以将当前工作区对应数据库的以下内容一起打包：
...
- 如果当前脚本或数据库存在未保存修改，应用会先提示保存，再继续打包
- 在数据库页会打包当前活动数据库
- 在脚本页会打包当前脚本所属的数据库
- 默认输出文件名为同目录同名 `.zip`
