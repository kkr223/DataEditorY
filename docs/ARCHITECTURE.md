...
| 卡图渲染 | `ygo-card-renderer-rs` | Rust 侧卡图渲染库；当前适配层已完成本轮核心重构 |
...
| `appSettingsRequest.ts` | 保存设置请求构造与设置字段规范化，便于独立测试密钥保存/清除请求 |
...
当前状态：渲染链路已用 Rust DTO 自动生成 TypeScript 类型；app/cdb 相关 settings、script、package、merge、基础 CDB 查询/修改 DTO 已按同一模式接入生成类型。直接 primitive 参数类命令仍保持轻量手写调用。
...
| 现状 | 影响 | 建议 |
| --- | --- | --- |
| `RenderCardPayload` 已从 `unknown` 改为应用级 DTO，并由 Rust `ts-rs` 生成 TypeScript 类型；共享 JSON fixture 同时覆盖 TypeScript 序列化与 Rust 反序列化 | 渲染链路字段漂移已被生成类型与 fixture 双重约束 | 继续保留关键 JSON fixture，并扩展更多视觉样例 |
| settings、script、package、merge、基础 CDB 查询/修改相关 app/cdb DTO 已由 Rust `ts-rs` 生成 `generated/app.ts`、`generated/cdb.ts` | 命令 wrapper 和核心 store 不再维护这些响应/请求字段的手写副本 | 后续新增复杂 IPC DTO 时继续沿用生成类型 + freshness 测试 |
| 直接参数命令仍有少量 primitive 参数（如 `tabId`、`path`、`cardId`） | 字段数量低，漂移风险低 | 统一约定：前端保持 camelCase，Rust DTO 一律显式 `#[serde(rename_all = "camelCase")]`，复杂请求优先建 DTO |
...
| 现状 | 影响 | 建议 |
| --- | --- | --- |
| 打包/合并已用 event 通知进度；`render_card` 已改为 async command 并通过 `spawn_blocking` 在 Rust 阻塞线程池执行，前端 API 仍保持 Promise 返回 PNG bytes | 预览渲染不再占用 Tauri command handler 的同步执行路径；但结果仍通过 invoke 返回完整 PNG byte array | 暂不引入 event-result 协议；若后续需要取消、排队、进度或更大图传输，再升级为 task id + event 完成通知，并同时评估 bytes/file token 传输方式 |

### 9.11 缺少跨模块的共享类型文件

| 现状 | 影响 | 建议 |
| --- | --- | --- |
| `CardDataEntry`、CDB IPC DTO 在 `types/cdb.ts`，脚本文件信息在 `types/script.ts`，渲染 DTO 在 `types/render.ts`，app IPC DTO 在 `types/app.ts` | 核心跨模块类型已有主归属；剩余局部 UI 类型仍可按收益逐步收敛 | 继续保持 card data、script document、render draft、settings/merge/package DTO 等跨模块类型集中在 `types/` 或 `domain/` 下 |

### 9.12 测试覆盖边界集中在纯逻辑

| 现状 | 影响 | 建议 |
| --- | --- | --- |
| 测试覆盖 `domain/`、部分 `application/`、少量 `features/`；Rust services 已补 CDB 合并冲突计划、资源 winner/include flag、package Lua 递归依赖/循环/越界保护；前端已补 settings 保存请求构造测试 | 最复杂的服务边界已有更明确回归保护；IPC 层和 UI 交互仍偏薄 | 后续优先补更多渲染卡种/语言样例，以及必要的 UI 交互 smoke |

### 9.13 优化优先级分类

| 优先级 | 项目 | 理由 |
| --- | --- | --- |
| 高（已完成本轮目标） | IPC 类型契约（9.1） | 渲染、settings、script、package、merge、基础 CDB 查询/修改 DTO 已建立 `ts-rs` 生成链路 |
...
| 中（已完成第一步） | 异步渲染（9.10） | `render_card` 已进入 async command + `spawn_blocking`；event-result 模式留待出现取消/进度/大图传输需求时再做 |
