...
- **Search-based condition** (e.g. "all cards with ATK > 2000", "all Dragon monsters"): use `search_cards` with an appropriate query to find target cards, paginating with `page`/`limit` if needed.
...
2. Confirm with the user the intended field-level change (e.g. "set atk to 1500 for all 12 matched cards") before proposing — unless the instruction is unambiguous.
